...
const FlashcardManager = ({documentId}) => {
    ...
    const handleGenerateFlashcards= async()=> {
        ...
        try {
            ...
            toast.error(error.messase || 'Failed to generate flashcards')
            console.error(error)
        } finally {
            setGenerating(false)
        }
        ...
        return (
            <div className='space-y-6'>
                ...
                <button 
                    onClick={handleGenerateFlashcards}
                    disabled={generating}
                    className='group inline-flex items-center gap-2 px-5 h-11 bg-linear-to-r from-emerald-500 to-teal-500 text-white hover:from-emerald-600 hover:to-teal-600 font-semibold text-sm rounded-xl shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 transition-all duration-200'
                >
                    {generating ? (
                        <>
                            <div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin' />
