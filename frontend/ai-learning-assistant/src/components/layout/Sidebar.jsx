...
const Sidebar = ({isSidebarOpen, toggleSidebar}) => {
    ...
    return <>
        <div className={`fixed inset-0 bg-black/30 z-40 md:hidden transition-opacity duration-300 ${
            isSidebarOpen ? 'opacity' : 'opacity-0 pointer-events-none'
        }`}
        onClick={toggleSidebar}
        aria-hidden= 'true'
        ></div>

        <aside className= {`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:w-64 md:shrink-0 md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${
            ...
            <nav className='flex-1 px-3 py-6 space-y-1.5'>
                {navLinks.map((link)=> (
                    <NavLink
                        ...
                        onClick={toggleSidebar}
                        className= {({isActive})=> 
                            `group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transiiton-all duration-200 ${
                                isActive
